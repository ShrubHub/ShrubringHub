...
# Load data
load("workspace/growth_full.RData")
...
# Merge data frames
clim_data <- merge(climate_data, climate_datan)
clim_data$dataset <- substring(clim_data$site, 1, 1)                       
clim_data <- subset(clim_data, dataset == "s")
growth.full$rw <- NULL
growth <- subset(growth.full, year > 1949)
data <- merge(growth, clim_data, by.x = c("site_id", "year"), by.y = c("site", "year"))
data[data == "NaN"] <- "NA"
...
# Print prop
save(propn, file="scripts/users/imyerssmith/shrub_synthesis/rand/propn.RData")
write.csv(propn, file = "scripts/users/imyerssmith/shrub_synthesis/rand/propn.csv", row.names=FALSE)

save(lmmodelsn, file="scripts/users/imyerssmith/shrub_synthesis/rand/lmmodelsn.RData")
write.csv(lmmodelsn, file = "scripts/users/imyerssmith/shrub_synthesis/rand/lmmodelsn.csv", row.names=FALSE)
...
# Print models
save(modelsn, file="scripts/users/imyerssmith/shrub_synthesis/rand/modelsn.RData")
write.csv(modelsn, file = "scripts/users/imyerssmith/shrub_synthesis/rand/modelsn.csv", row.names=FALSE)
...
save(model_compn, file="scripts/users/imyerssmith/shrub_synthesis/rand/model_compn.RData")
write.csv(model_compn, file = "scripts/users/imyerssmith/shrub_synthesis/rand/model_compn.csv", row.names=FALSE)
...
for (a in 1:100) {
  ...
  for (i in 1:length) { 
    data <- subset(ndata, year >= 1950) %>% ddply(., .(shrub_num), transform, rand = sample(nrw)) 
    data <- subset(data, sgnum == i)    
...
save(randomn, file="scripts/users/imyerssmith/shrub_synthesis/rand/randomn.RData")
write.csv(randomn, file = "scripts/users/imyerssmith/shrub_synthesis/rand/randomn.csv", row.names=FALSE)
